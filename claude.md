...
| Auth                | better-auth                   | Org auth + multi-tenancy (organization plugin), configured in `apps/api`, consumed from `apps/frontend` |
...
```
/apps
  /frontend        → Next.js, org-facing UI
...
- **frontend-design** — required for any work touching `apps/dashboard` or `packages/ui`. This dashboard is an operational tool for small business owners checking whether their AI phone agent is working — not a marketing site. Do not default to generic component-library styling (cream+terracotta, dark+neon-accent, identical rounded cards). Every dashboard feature session starts in plan mode with a design-token pass reviewed against generic-AI-design tells before any code is written.
...
- `apps/dashboard` and `packages/ui` work follows plan → critique → build, not straight to code.
